<template>
  <div style="border:1px solid black; padding: 25px; margin-bottom: 50px; text-align:center;">
      ...
      <canvas id="TimeslotUsageWeekdaysChart" width="1080" height="650" :style="chartVisibility"></canvas>
  </div>
</template>
...
export default {
  name: 'TimeslotUsageWeekdaysChart',
  data() {
    return {
      mapTitle: '',
      courtTotalsData: [],
  ...
  methods: {
    ...
    async fetchData(year) {
      let urlAddress = `${import.meta.env.VITE_MONGODB_URI}/?DayType=Weekdays`;
      if (year) {
          urlAddress = `${import.meta.env.VITE_MONGODB_URI}/getTimeslotCount?DayType=Weekdays&Year=${year}`;
      }
    ...
    async configureChart(year) {
        ...
        this.mapTitle = 'Weekday Timeslot Usage (All-time)';
        if (year) {
            this.mapTitle = `Weekday Timeslot Usage (${year})`;
        }
      ...
      this.currentChart = new Chart(
          document.getElementById('TimeslotUsageWeekdaysChart'),
          this.chartConfig
      );
    },
  },
